defmodule AwsIotClient do
  ...
  @options [
    name: __MODULE__,
    # host: "a198unk2hf0hqn-ats.iot.us-east-1.amazonaws.com",
  ...
  def start_link do
    ...
    Logger.debug("starting MQTT client with options: #{inspect(start_options)}")
  ...
  def on_connect(%{subscribe_queue: subscribe_queue, subscriptions: subscriptions} = state) do
    Logger.info(
      "CONNECTED subscribe_queue length: #{inspect(length(subscribe_queue))}, subscriptions length: #{
        inspect(length(subscriptions))
      }"
    )

    new_subscriptions =
      Enum.reduce(subscribe_queue, subscriptions, fn {req, from}, acc ->
  ...
  def on_connect_error(reason, state) do
    Logger.error("GenMQTT connect error: #{inspect(reason)}")
    {:ok, state}
  end
  ...
  def handle_call(
        {:subscribe, topics, qos, retain} = req,
        from,
        %{subscribe_queue: subscribe_queue, ready: false} = state
      ) do
  ...
  def handle_call(
        {:subscribe, topics, qos, retain} = req,
        from,
        %{subscriptions: subscriptions, ready: true} = state
      ) do
